...
class AppDelegate: UIResponder, UIApplicationDelegate, UITabBarControllerDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        */
        ...
        //隐藏
        //tabCtrl.tabBar.hidden = true
        
        //设置当前选中哪一个
        let ud = NSUserDefaults.standardUserDefaults()
        let str = ud.objectForKey("lastTitle")
        if nil != str {
            
            let myStr = str as! String
            
            for tmp:UIViewController in tabCtrl.viewControllers! {
                let title = tmp.tabBarItem.title
                
                if title == myStr {
                    tabCtrl.selectedViewController = tmp
                    break
                }
    ...
    func tabBarController(tabBarController: UITabBarController, didSelectViewController viewController: UIViewController) {
        ...
        let str = viewController.tabBarItem.title
        ud.setObject(str, forKey: "lastTitle")
