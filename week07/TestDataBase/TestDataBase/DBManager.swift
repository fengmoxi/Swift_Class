...
class DBManager: NSObject {
    ...
    func createDataBase() {
        ...
        //2.关联数据库文件
       let filePath = NSHomeDirectory().stringByAppendingString("/Documents/user.sqlite")
    ...
    func insertUser(dict: Dictionary<String,AnyObject>) {
        ...
        */
        ...
        if dict.keys.contains(kAge) {
            entity.age = dict[kAge] as? NSNumber
        }
        if dict.keys.contains(kImage) {
            entity.image = dict[kImage] as? NSData
        }
    ...
    func updateUser(dict:Dictionary<String,AnyObject>, userId: NSNumber) {
        ...
        for u in ret {
            ...
            if dict.keys.contains(kAge) {
                u.age = dict[kAge] as? NSNumber
            }
            if dict.keys.contains(kImage) {
                u.image = dict[kImage] as? NSData
            }
        }
